...
def result():
	...
	#line_chart.width=100
    #line_chart.y_labels = map(str, [0,20,40,60,80,100])
    ...
    plt.bar(left, height, tick_label = tick_label,width = 200.0, color = ['red', 'green','yellow', 'blue','brown']) 
...
def sendmsg():
    if request.method=='POST':
        ...
        server.login("tanishporwal14dec@gmail.com", "toxpliyokbfjwjxt")
        server.sendmail(
                "tanishporwal14dec@gmail.com", 
                 eml, 
                 fig1)
        server.quit()
        return render_template("front.html")
